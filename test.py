...
class Agent():
    ...
    def load_model(self, path_model, path_target = None):
        params = torch.load(path_model, map_location=device)
        #self.network.set_params(params)
        self.network.load_state_dict(torch.load(path_model, map_location=device))
        if path_target != None:
            self.target_network.load_state_dict(torch.load(path_target, map_location=device))
